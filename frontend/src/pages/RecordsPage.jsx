...
import { deleteBooking, fetchBookings, setLastQuery, updateBooking } from '../store/bookingsSlice';
...
async function exportAllToExcel(query) {
...
export function RecordsPage() {
  ...
  const columns = useMemo(() => {
          ...
          <Select
            ...
            onChange={async (v) => {
              try {
                ...
                dispatch(fetchBookings({ ...lastQuery, page, pageSize }));
              } catch (e) {
                if (e?.data?.code === 'SCHEDULE_CONFLICT') message.error(conflictMessage(e?.data));
                else message.error(e?.message || '更新失败');
              }
            }}
          />
        ) : (
        ...
        <Space>
          ...
          {can('booking:delete') ? (
            <Popconfirm
              ...
              onConfirm={async () => {
                try {
                  await dispatch(deleteBooking(r.id)).unwrap();
                  message.success('已删除');
                  dispatch(fetchBookings({ ...lastQuery, page, pageSize }));
                } catch (e) {
    ...
    return cols;
  }, [canChangeStatus, canEditForm, dispatch, isTechnician, lastQuery, page, pageSize]);

  useEffect(() => {
    dispatch(fetchBookings({ page: 1, pageSize: 50 }));
  }, [dispatch]);
  ...
  useEffect(() => {
    ...
    api.listExperimenters()
      .then((r) => {
        if (!mounted) return;
        setExperimenters((r.items || []).filter((e) => e.isActive));
      })
      .catch(() => {
        if (!mounted) return;
        setExperimenters([]);
      ...
      <Card style={{ marginBottom: 12 }}>
        <Form
          ...
          onFinish={async (values) => {
            const visitFrom = values.visitRange?.[0] ? values.visitRange[0].format('YYYY-MM-DD 00:00:00') : undefined;
            const visitTo = values.visitRange?.[1] ? values.visitRange[1].format('YYYY-MM-DD 23:59:59') : undefined;
            const q = {
              salesName: values.salesName || undefined,
              contractNo: values.contractNo || undefined,
              customerUnit: values.customerUnit || undefined,
              customerName: values.customerName || undefined,
              needDissociation: values.needDissociation === undefined ? undefined : String(values.needDissociation),
              sampleInfo: values.sampleInfo || undefined,
            ...
            <Space>
              ...
              {can('booking:export') ? (
                <Button
                  onClick={async () => {
                    try {
                      await exportAllToExcel(lastQuery || {});
      ...
      <Card>
        ...
        <Table
          ...
          pagination={{
            ...
            onChange: (p, ps) => {
              const q = { ...(lastQuery || {}), page: p, pageSize: ps };
              dispatch(setLastQuery(q));
              dispatch(fetchBookings(q));
            }
          }}
      ...
      >
        <BookingForm
          ...
          onSubmit={async (payload) => {
            ...
            try {
              await dispatch(updateBooking({ id: editing.id, body: payload })).unwrap();
              message.success('已保存');
              setDrawerOpen(false);
              setEditing(null);
              dispatch(fetchBookings({ ...lastQuery, page, pageSize }));
