...
export function AnalyticsPage() {
  ...
  const seqData = useMemo(
    () => (data?.seqType || []).slice(0, 10).map((s) => ({ name: s.seqType, count: s.count })),
    [data]
  );
  const platformData = useMemo(
    () => (data?.platform || []).map((p) => ({ name: p.platform, count: p.count })),
    [data]
  );
  const trendData = useMemo(
    () => (data?.trend || []).map((t) => ({ day: String(t.day).slice(5), count: t.count })),
    [data]
  );
  const personalTrendData = useMemo(
    () => (data?.personal?.trend || []).map((t) => ({ day: String(t.day).slice(5), count: t.count })),
    [data]
  );
  const personalSeqData = useMemo(
    () => (data?.personal?.seqType || []).slice(0, 10).map((s) => ({ name: s.seqType, count: s.count })),
    [data]
  );
  ...
  return (
    <div>
      ...
      {errMsg ? <Alert type="error" showIcon message={errMsg} style={{ marginBottom: 12 }} /> : null}
      ...
      <Card title="简要分析" loading={loading}>
        <div className="muted leading-relaxed">{data?.insight || '暂无分析内容'}</div>
      </Card>

      {data?.personal ? (
        <Card title={`个人统计（${data.personal.experimenter}）`} loading={loading} style={{ marginTop: 12 }}>
          <div className="cardGrid mb-3">
            <Card size="small" title="个人总订单量">
              {data.personal.total || 0}
            </Card>
            <Card size="small" title="个人完成订单">
              {data.personal.done || 0}
            </Card>
            <Card size="small" title="个人未完成订单">
              {data.personal.undone || 0}
            </Card>
          </div>
