...
import { api } from '../api';
import type { ExperimenterRow, RoleRow, UserRow } from '../api';
import { getHttpError } from '../lib/http-error';

export function UsersPage() {
  const [items, setItems] = useState<UserRow[]>([]);
  const [roles, setRoles] = useState<RoleRow[]>([]);
  const [experimenters, setExperimenters] = useState<ExperimenterRow[]>([]);
  ...
  const openTransferModal = (user: UserRow) => {
                  ...
                  <Popconfirm
                    ...
                    onConfirm={async () => {
                      try {
                        ...
                        if (he.data?.code === 'USER_HAS_ACTIVE_ORDERS') {
