...
function pendingOverdueDays(serviceEndTime: unknown) {
  const end = dayjs(serviceEndTime as string).startOf('day');
...
function overdueStyle(days: number) {
  ...
  if (days <= 2) {
    return { color: '#389e0d', fontWeight: 600 };
  }
  if (days <= 5) {
    return { color: '#d46b08', fontWeight: 600 };
  }
...
export function DashboardPage() {
  const { data: bookingsData, isPending: bookingsLoading } = useQuery({
    queryKey: ['bookings', 'list', { page: 1, pageSize: 200 }],
    queryFn: () => api.listBookings({ page: 1, pageSize: 200 })
  });
  ...
  const groups = useMemo(() => {
    const map: Record<string, BookingRow[]> = { pending: [], in_progress: [], done: [] };
  ...
  const totals = useMemo(() => {
    ...
    for (const [day, count] of dailyMap.entries()) {
      ...
      if ((d.isAfter(weekStart) || d.isSame(weekStart, 'day')) && (d.isBefore(weekEnd) || d.isSame(weekEnd, 'day'))) {
        weekCount += count;
      }
      if ((d.isAfter(monthStart) || d.isSame(monthStart, 'day')) && (d.isBefore(monthEnd) || d.isSame(monthEnd, 'day'))) {
        monthCount += count;
      }
    }
        ...
        {STATUS_OPTIONS.map((s) => (
          ...
          >
            <List
              ...
              renderItem={(item) => {
                const od = s.value === 'pending' ? pendingOverdueDays(item.serviceEndTime) : 0;
                  ...
                  <List.Item>
                    <div style={{ width: '100%', ...panelTextStyle }}>
                      <div style={{ fontWeight: 600, color: '#000' }}>
                        ...
                        {od > 0 ? (
                          <span style={{ marginLeft: 8, ...odStyle }}>逾期 {od} 天</span>
                        ) : null}
                      </div>
            ...
            {(groups[s.value]?.length || 0) > 8 ? (
              <div style={{ marginTop: 8, fontSize: 12, color: '#000' }}>仅展示前 8 条，完整列表请到「预约记录」。</div>
            ) : null}
          </Card>
        ))}
      </div>
