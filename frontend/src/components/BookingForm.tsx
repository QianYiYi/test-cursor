...
import type { BookingPayload, ExperimenterRow, PmOwnerRow } from '../api';
import { PLATFORMS, SEQ_TYPES } from '../constants';

const { RangePicker } = DatePicker;

/** 多日出差：仅选「上门服务时刻」；结束时间 = 上门后 1 小时，不超过出差结束、且不跨自然日后再截断 */
function computeServiceEndTime(visitAt: Dayjs | null | undefined, tripEnd: Dayjs | null | undefined) {
...
function formatDt(d: Dayjs | null | undefined) {
  return d ? d.format('YYYY-MM-DD HH:mm:ss') : '';
}

function normalizeMobile(s: unknown) {
  return String(s ?? '').replace(/\s+/g, '');
}

export interface BookingFormProps {
  initialValues?: (Record<string, unknown> & { id?: number; tripStart?: string; tripEnd?: string }) | null;
  onSubmit?: (payload: BookingPayload) => void | Promise<void>;
  submitting?: boolean;
  submitText?: string;
  experimenters?: ExperimenterRow[];
...
export function BookingForm({
  ...
  useEffect(() => {
    ...
    if (isLegacyEdit) {
      form.setFieldsValue({
        ...initialValues,
        visitTime: initialValues.visitTime ? dayjs(initialValues.visitTime as string) : null,
        serviceEndTime: initialValues.serviceEndTime ? dayjs(initialValues.serviceEndTime as string) : null
      });
      return;
    }
    const ts = initialValues.tripStart ? dayjs(initialValues.tripStart as string) : null;
    const te = initialValues.tripEnd ? dayjs(initialValues.tripEnd as string) : null;
    form.setFieldsValue({
      ...initialValues,
      tripRange: ts && te ? [ts, te] : undefined,
      visitAt: initialValues.visitTime ? dayjs(initialValues.visitTime as string) : undefined
    });
